...
let DIGIT_RE = /^\d$/;
let DIGIT_SPACE_RE = /^[\d ]$/;
let LETTER_RE = /^[A-Za-z]$/;
let ALPHANNUMERIC_RE = /^[\dA-Za-z]$/;

let DEFAULT_PLACEHOLDER_CHAR = '_';
let DEFAULT_FORMAT_CHARACTERS = {
  ' ': {
    validate: function(char) { return DIGIT_SPACE_RE.test(char) }
  },
  '*': {
...
Pattern.prototype._parse = function parse() {
  let sourceChars = this.source.split('');
  let patternIndex = 0;
  let pattern = [];
...
Pattern.prototype.formatValue = function format(value) {
  let valueBuffer = new Array(this.length);
  let valueIndex = 0;
  ...
  for (i = 0, l = input.length;
    ...
    if (!valid) {
      if (this.selection.start > 0) {
        ...
        let patternIndex = this.selection.start - 1;
